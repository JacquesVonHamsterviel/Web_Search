...
require 'global.php';
headhtml();
;echo '<div class="nav" style="display:;"><a href="?action=add&p_cid=';echo $p_cid;echo '">添加顶级分类</a><a href="?action=create_cache">生成缓存</a></div>
';
$action=$_GET['action'];
switch ($action)
{
...
;echo '    </select>	</td>
  ...
  <tr style="display:none;">
    <td>链接参数:</td>
    <td><input name="cate_url" type="text" size="80" value="';echo $cate_url;echo '"/>
      (如果你不是程序员,请留空)</td>
  </tr>
  <tr>
    <td width="100">分类名称:</td>
    <td>
      <textarea name="cate_title" cols="80" rows="10">';echo $cate_title;echo '</textarea>
      (支持批量添加,一行一个!)</td>
  </tr>
  <tr>
    <td>排序:</td>
  ...
  <tr>
    <td>&nbsp;</td>
    <td>
	<input type="hidden" name="cate_id" value="';echo $cate_id;echo '">
...
$str.=' ?>';
$fp=@fopen('../include/cache/s_cate_array.php','w') or die('写方式打开文件失败，请检查程序目录是否为可写');
...
$str_menu.=' ?>';
$fp=@fopen('../include/cache/s_cate_menu_array.php','w') or die('写方式打开文件失败，请检查程序目录是否为可写');
