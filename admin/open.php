...
$db->query("delete from ve123_open where about_id='".$about_id."'");
break;
}
;echo '<table width="100%" border="0" cellpadding="3" cellspacing="1" class="tablebg">
  ...
  ';
$result=$db->query('select * from ve123_open order by about_id desc');
while ($rs=$db->fetch_array($result))
{
;echo '  <tr>
    <td>';echo $rs['about_id'];;echo '</td>
    <td>';echo $rs['filename'];;echo '</td>
    <td>';echo "<a target=\"_blank\" href=\"".$site['url'].'/open/html/'.$rs['filename'].".html\" title=\"查看".$rs['title']."\">".$rs['filename'].'.html';;echo '</a></td>
...
$sql="select * from ve123_open where about_id='$about_id'";
$rs=$db->get_one($sql);
$title=$rs['title'];
$content=$rs['content'];
...
file_put_contents('../open/html/'.$filename.'.html',$str);
if ($do_action=='modify')
{
$array=array('title'=>$title,'content'=>$content,'url'=>$url,'filename'=>$filename,'sortid'=>$sortid,'is_show'=>$is_show);
$db->update('ve123_open',$array,"about_id='$about_id'");
jsalert('修改成功');
}
else
{
$array=array('title'=>$title,'content'=>$content,'url'=>$url,'filename'=>$filename,'sortid'=>$sortid,'is_show'=>$is_show);
$db->insert('ve123_open',$array);
jsalert('提交成功');
}
}
;echo '
