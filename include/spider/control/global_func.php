   ...
   for($i=0;$i<=$he_num;$i++) 
	 {
		...
		foreach ((array)$fen_url as $url => $file)
  	 	{ 
					...
					$file  = Convert_File($file,$bianma);		 //转换所有编码为utf-8	
...
function cmi($links,$killspace=TRUE,$forhtml=TRUE,$timeout=6,$header=0,$follow=1){
    		...
    		foreach ((array)$links as $i => $url) {
    			 $conn[$url]=curl_init($url);//若url中含有utf-8汉字，例如FTP时，要在传入url的时候处理一下，这里不用
...
function Convert_File($file,$charSet)
{
				 ...
				 {                    
                        $file=convertfile($charSet,"utf-8",$conv_file);
						if($file==-1){ return -1; }
                 }  
				return $file;
}
...
function foothtml()
{
echo "<div style=\"text-align:center;\"><a target=\"_blank\" href=\"https://www.google.com\">Powered by Google</a></div>";
}
?>
